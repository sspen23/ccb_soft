...
int main(void)
{
    ...
    e.arg0 = 3; storage_event_ring_push(&r, &e, 0);
    n = storage_event_ring_copy(&r, out, 4u);
    assert(n == 3u && out[0].sequence < out[1].sequence && out[1].sequence < out[2].sequence);
    assert(r.fatal_sequence != 0u && r.fatal.arg0 == 1u);
    assert(atomic_load(&r.overwrite_count) == 2u);
    assert(storage_event_ring_should_dump(true, false, true, false));
    assert(storage_event_ring_should_dump(false, true, false, true));
    assert(!storage_event_ring_should_dump(false, true, true, false));
