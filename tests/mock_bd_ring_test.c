...
static void verify_completed_unharvested_survives_quiesce(uint32_t completed)
{
    ChannelRuntime rt;
    ChannelConfig cfg;
    DmaSgDesc desc[DMA_DESC_COUNT_MAX];
    uint32_t dma_regs[0x100u / 4u];
    uint8_t state[DMA_DESC_COUNT_MAX];
    DmaStopReport stop_report;
    DmaHarvestItem harvested[DMA_DESC_COUNT_MAX];
    uint32_t harvested_count = 0u;
    uint32_t i;

    assert(completed <= DMA_DESC_COUNT_MAX);
    ...
    rt.dma_hw_desc_count = DMA_DESC_COUNT_MAX;
    dma_regs[TEST_S2MM_TAILDESC / 4u] =
        (uint32_t)(cfg.desc_dma_base +
                   (DMA_DESC_COUNT_MAX - 1u) * sizeof(DmaSgDesc));
    ...
    assert(dma_harvest_completed_batch(&rt, harvested, DMA_DESC_COUNT_MAX,
                                       &harvested_count) == 0);
...
int main(void)
{
    ...
    assert(CHANNEL0_DDR_BYTES == 2ull * 1024ull * 1024ull * 1024ull);
    assert(CHANNEL1_DDR_BYTES == 2ull * 1024ull * 1024ull * 1024ull);
    assert(DMA_DESC_COUNT_MAX == 256u);

    init_runtime(&rt, &cfg, desc, dma_regs);
    cfg.desc_cpu_size = 0x4000u;
    cfg.ddr_hw_base = 0u;
    rt.dma_ring_bytes = CHANNEL0_DDR_BYTES;
    rt.gopt.dry_run = true;
    assert(dma_prepare_s2mm_ring(&rt, 8u * 1024u * 1024u) == 0);
    assert(rt.dma_desc_count == 256u && rt.dma_hw_desc_count == 256u);
    ...
    verify_completed_unharvested_survives_quiesce(DMA_DESC_COUNT_MAX);
