...
{ if (!s->first_fatal) { s->first_fatal = true; s->fatal_channel = ch; snprintf(s->fatal_reason, sizeof(s->fatal_reason), "%s", reason); } s->stop_requested_mask |= s->target_channel_mask & ~s->final_seen_mask; s->terminal = STORAGE_TASK_FAILED; }
...
{ if (!s || !(s->final_seen_mask & bit(ch))) { if (s) { fail(s,ch,"event_eof_without_final"); (void)storage_supervisor_result_status(s); } return -1; } return 0; }
...
StorageTaskTerminal storage_supervisor_result_status(StorageTaskSupervisor *s)
{
    ...
    if (s->first_fatal) return s->terminal = STORAGE_TASK_FAILED;
    if (s->final_seen_mask != s->target_channel_mask) return STORAGE_TASK_ACTIVE;
