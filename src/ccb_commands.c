...
typedef enum {
    STORAGE_SLOT_FREE = 0,
    STORAGE_SLOT_DMA_OWNED = 1,
    STORAGE_SLOT_DMA_DONE_READY = 2,
...
typedef struct {
    ...
    uint32_t free_slots;
    uint32_t dma_owned_slots;
    uint32_t dma_done_ready_slots;
...
static void storage_apply_writer_rt(StorageWriteQueue *q)
{
    ...
    if (rc == 0) {
        q->writer_rt_enabled = true;
    } else {
        fprintf(stderr,
                "warning: writer RT priority setup failed channel=%d prio=%u errno=%d\n",
                q->rt->cfg->id,
                (unsigned)prio,
                rc);
    }
...
static int storage_queue_init(StorageWriteQueue *q,
    ...
    memset(q->slot_state, STORAGE_SLOT_DMA_OWNED, rt->dma_desc_count);
    q->rt = rt;
...
static int storage_queue_push(StorageWriteQueue *q,
    ...
    while (!q->error && q->count == q->capacity) {
    ...
    if (q->slot_state) {
        q->slot_state[slot] = STORAGE_SLOT_DMA_DONE_READY;
    }
...
static void storage_queue_snapshot(StorageWriteQueue *q, StorageQueueSnapshot *out) {
    ...
    out->writer_rt_enabled = q->writer_rt_enabled;
    out->writer_rt_prio = q->writer_rt_prio;
    for (i = 0u; i < q->capacity; ++i) {
        switch (q->slot_state ? q->slot_state[i] : STORAGE_SLOT_FREE) {
        case STORAGE_SLOT_FREE:
            ++out->free_slots;
            break;
        case STORAGE_SLOT_DMA_OWNED:
            ++out->dma_owned_slots;
            break;
        case STORAGE_SLOT_DMA_DONE_READY:
            ++out->dma_done_ready_slots;
            break;
...
static void storage_stats_print_periodic(StorageProducerStats *stats,
    ...
    if (!print_zero_stats && received_delta == 0u && nvme_delta == 0u) {
        if (!stats->idle_printed && received_bytes == 0u && nvme_bytes == 0u) {
            storage_emit_line("storage_pipeline_idle channel=%d dma_writable_slots=%u",
                   rt->cfg->id,
                   (unsigned)snapshot.dma_owned_slots);
    ...
    storage_emit_line("storage_pipeline channel=%d window_ms=%" PRIu64 " task=%s file_index=%u"
           ...
           (unsigned)snapshot.dma_done_ready_slots,
...
static int storage_complete_slot(StorageWriteQueue *q, const PendingDdrSlot *item) {
    ...
    if (q->slot_state) {
        q->slot_state[item->slot] = should_requeue ? STORAGE_SLOT_DMA_OWNED
                                                   : STORAGE_SLOT_FREE;
    }
    pthread_mutex_unlock(&q->lock);

    if (should_requeue) {
        dma_requeue_one(q->rt, item->slot);
    }
    return 0;
}
...
static int storage_queue_pop(StorageWriteQueue *q, PendingDdrSlot *out, bool wait_for_item) {
    ...
    *out = q->items[q->head];
    ...
    --q->count;
    if (q->slot_state && out->slot < q->capacity) {
        q->slot_state[out->slot] = STORAGE_SLOT_NVME_BUSY;
    }
    storage_queue_record_depth_locked(q);
...
static void *storage_nvme_cross_slot_writer_thread(void *arg) {
    StorageWriteQueue *q = (StorageWriteQueue *)arg;
    uint32_t batch_limit = q && q->cross_slot_batch > 0u ? q->cross_slot_batch
                                                         : NVME_CROSS_SLOT_BATCH_DEFAULT;
    PendingDdrSlot *items = NULL;
    NvmeWriteSlotReq *reqs = NULL;

    if (!q) {
        return NULL;
    }
    storage_apply_writer_rt(q);
    if (batch_limit > q->capacity) {
        batch_limit = q->capacity;
    }
    if (batch_limit == 0u) {
        batch_limit = 1u;
    }
    items = (PendingDdrSlot *)calloc(batch_limit, sizeof(items[0]));
    reqs = (NvmeWriteSlotReq *)calloc(batch_limit, sizeof(reqs[0]));
    if (!items || !reqs) {
        free(items);
        free(reqs);
        storage_mark_writer_error(q);
    ...
    while (1) {
        ...
        wait_start_us = storage_wall_time_us();
        pop_rc = storage_queue_pop(q, &items[0], true);
        if (pop_rc > 0) {
            (void)__atomic_add_fetch(&q->writer_idle_us,
                                     storage_elapsed_us(wait_start_us),
                                     __ATOMIC_RELEASE);
        ...
        req_count = 1u;
        while (req_count < batch_limit) {
            pop_rc = storage_queue_pop(q, &items[req_count], false);
            if (pop_rc <= 0) {
                break;
            }
            ++req_count;
        ...
        for (i = 0u; i < req_count; ++i) {
            storage_trace_flush_start(q->rt, &items[i]);
            reqs[i].slot = items[i].slot;
            reqs[i].start_lba = items[i].start_lba;
            reqs[i].sectors = items[i].sectors;
            reqs[i].hw_addr = items[i].hw_addr;
            reqs[i].bytes = items[i].bytes;
            reqs[i].chunk_index = items[i].chunk_index;
            reqs[i].file_offset = items[i].file_offset;
        }
        write_start_us = storage_wall_time_us();
        if (nvme_write_slots_qd(q->rt,
                                reqs,
                                req_count,
                                storage_cross_slot_done_cb,
                                q) != 0) {
            storage_mark_writer_error(q);
            break;
        }
        ...
        (void)__atomic_add_fetch(&q->writer_active_us, write_us, __ATOMIC_RELEASE);
        (void)__atomic_add_fetch(&q->writer_drain_loop_count, 1u, __ATOMIC_RELEASE);
        (void)__atomic_add_fetch(&q->writer_slots_drained, req_count, __ATOMIC_RELEASE);
        if (storage_queue_ready_count(q) > 0u || q->backlog_mode) {
            (void)__atomic_add_fetch(&q->ready_q_nonempty_us, write_us, __ATOMIC_RELEASE);
        }
    }
    free(items);
...
int execute_write_with_result(const ParsedArgs *args, GlobalOptions gopt, WriteResult *result) {
    ...
    storage_poll_sleep_us = storage_env_u32_limit("SRC_REAL_STORAGE_POLL_SLEEP_US",
                                                  STORAGE_POLL_SLEEP_US_DEFAULT,
                                                  1000000u);
    ...
    ready_queue_depth_cfg = storage_env_u32_limit("SRC_REAL_READY_QUEUE_DEPTH",
                                                  ...
                                                  4096u);
    harvest_batch_max_cfg = storage_env_u32_limit("SRC_REAL_HARVEST_BATCH_MAX",
                                                  STORAGE_HARVEST_BATCH_MAX_DEFAULT,
                                                  1024u);
    cross_slot_qd = storage_env_flag_enabled("SRC_REAL_NVME_CROSS_SLOT_QD") != 0;
    cross_slot_batch = storage_env_u32_limit("SRC_REAL_NVME_CROSS_SLOT_BATCH",
                                             NVME_CROSS_SLOT_BATCH_DEFAULT,
                                             32u);
    ...
    if (dma_init_s2mm_ring(&rt, dma_desc_bytes) != 0) {
    ...
    dbg_verbose_printf("[DBG][WRITE] dma ready ch=%d desc_bytes=%u desc_count=%u desc_cpu=0x%08" PRIx64
                       ...
                       rt.dma_ring_bytes,
                       bounded ? 0u : 1u);
    printf("storage_ready channel=%d task=%s file_index=%u size=%" PRIu64 " continuous=%u\n",
           cfg->id,
           args->task_no,
           (unsigned)effective_file_index,
           requested_size,
    ...
    storage_emit_line("storage_pipeline_config channel=%d mode=%s fast_pipeline=%u"
                      ...
                      " qd=%u cmd_size=%u log_level=%u worker_rt_prio=%u"
    ...
    producer_stats.window_start_us = storage_wall_time_us();
    producer_stats.next_log_us = producer_stats.window_start_us +
                                 (uint64_t)producer_stats.interval_ms * 1000ull;
    {
        ...
        while (!bounded || bytes_captured < requested_size) {
            ...
            bool stop_requested = storage_write_stop_requested() != 0;
            ...
            if (h == 0) {
                ...
                storage_maybe_log_watermark(&producer_stats,
                                            ...
                                            effective_file_index,
                                            dma_received_bytes);
                if (busy_slots == rt.dma_desc_count) {
                    ...
                    if (!producer_stats.ring_warning_emitted ||
                        ...
                        storage_emit_line("storage_ring_warning channel=%d ring_full_count=%" PRIu64
                                          ...
                                          " integrity_risk=ddr_ring_full_no_aurora_backpressure",
                                          cfg->id,
                ...
                if (!bounded && !stop_requested && dma_idle_done_ms > 0u &&
                    ...
                    if (idle_us >= (uint64_t)dma_idle_done_ms * 1000ull) {
                        ...
                        storage_emit_line("storage_dma_idle_done channel=%d idle_ms=%" PRIu64
                                          ...
                                          (unsigned)idle_snapshot.dma_done_ready_slots);
                        auto_idle_done = true;
                        break;
                    }
                }
                if (stop_requested) {
                    ...
                    if (rt.dma_desc_count > 0u &&
                        busy_slots * 100u >= rt.dma_desc_count * 93u) {
                        sleep_us = storage_critical_poll_sleep_us;
                    } else if (rt.dma_desc_count > 0u &&
                               busy_slots * 100u >= rt.dma_desc_count * 75u) {
           ...
           bounded ? 0u : 1u);
    {
        ...
        uint64_t harvest_batch_avg = producer_stats.dma_harvest_batches > 0u
                                         ...
                                         : 0u;
        bool integrity_ok = !producer_stats.integrity_risk_ring_full && !tail_incomplete &&
                            !dma_stop_failed &&
