#ifndef STORAGE_CONFIG_H
#define STORAGE_CONFIG_H

#include "ccb_types.h"

#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>

#define STORAGE_CONFIG_UART_PATH_MAX 256u

/* Hardware-safety and production-path policies are compile-time constants.
 * They are intentionally not environment overrides. */
#define CCB_DDR_PADDING_DEVICE_COHERENT 0
#define CCB_ALLOW_ZERO_BYTE_CAPTURE 0
#define CCB_RING_CRITICAL_GRACEFUL_STOP 0
#define CCB_PERF_LOG_FSYNC 0
#define CCB_LEGACY_STORAGE_TEXT 0

#define CCB_INTERNAL_START_FD "CCB_INTERNAL_START_FD"
#define CCB_INTERNAL_STORAGE_CONTROL_FD "CCB_INTERNAL_STORAGE_CONTROL_FD"
#define CCB_INTERNAL_STORAGE_EVENT_FD "CCB_INTERNAL_STORAGE_EVENT_FD"
#define CCB_INTERNAL_SUPERVISED_CHANNEL_COUNT \
    "CCB_INTERNAL_SUPERVISED_CHANNEL_COUNT"
#define CCB_INTERNAL_STORAGE_WORKER "CCB_INTERNAL_STORAGE_WORKER"

typedef enum {
    CCB_LOG_ERROR = 0,
    CCB_LOG_INFO,
    CCB_LOG_PERF,
    CCB_LOG_DEBUG
} CcbLogLevel;

typedef enum {
    STORAGE_PROFILE_SAFE_QD1 = 0,
    STORAGE_PROFILE_LEGACY_FAST_BASELINE,
    STORAGE_PROFILE_PERF_QD8,
    STORAGE_PROFILE_CROSS_SLOT_EXPERIMENTAL
} StorageProfile;

typedef enum {
    STORAGE_WRITER_LEGACY = 0,
    STORAGE_WRITER_CROSS_SLOT
} StorageWriterMode;

typedef struct {
    uint32_t channel;
    StorageWriterMode writer_mode;
    uint64_t ring_bytes;
    uint32_t descriptor_bytes;
    uint32_t command_bytes;
    uint32_t nvme_qd;
    uint32_t max_active_slots;
    uint32_t cq_batch;
    bool writer_realtime;
    uint32_t writer_priority;
    bool producer_realtime;
    uint32_t producer_priority;
    uint32_t nominal_input_mib_s;
    uint32_t writer_scheduler_weight;
    uint32_t producer_scheduler_weight;
    int32_t writer_nice;
    int32_t producer_nice;
    uint32_t writer_budget_us;
    uint32_t busy_poll_us;
    uint32_t empty_sleep_us;
} ChannelStorageConfig;

typedef struct {
    char uart_device[STORAGE_CONFIG_UART_PATH_MAX];
    CcbLogLevel log_level;
    bool perf_enabled;
    uint32_t perf_interval_ms;
    uint32_t status_timeout_ms;
    uint32_t first_data_timeout_ms;
    StorageProfile storage_profile;
    bool legacy_compat_mode;
    bool dump_diag_on_error;
    bool auto_input_complete;
    uint32_t idle_scan_interval_ms;
    uint32_t idle_required_ms;
    uint32_t idle_required_scans;
    uint32_t drain_stable_scans;
    uint32_t drain_stable_us;
    ChannelStorageConfig channels[NUM_CHANNELS];
} AppConfig;

/* Load one immutable configuration snapshot. */
int storage_config_load(AppConfig *out, char *error, size_t error_size);
int storage_config_load_global(void);
const AppConfig *storage_config_get(void);
const ChannelStorageConfig *storage_config_channel(const AppConfig *config,
                                                   uint32_t channel);
const char *storage_config_profile_name(StorageProfile profile);
const char *storage_config_log_level_name(CcbLogLevel level);
bool storage_config_legacy_compat_enabled(void);

/* Transitional compatibility entry point.  Process-environment access is
 * confined to storage_config.c while old overrides move to the snapshot. */
const char *storage_config_compat_getenv(const char *name);

#endif
